...
def login_attempt(request):
    if request.method == 'POST':
        ...
        if user_obj is None:
            messages.success(request, 'User details not found.')
        ...
        if not profile_obj.is_verified:
            messages.success(request, 'Profile is not verified yet.Please check your email.')
            return redirect('/accounts/login')
...
def send_mail_after_registration(email , token):
    ...
    message = f'Paste the link in the browser to activiate your account http://127.0.0.1:8000/verify/{token}'
